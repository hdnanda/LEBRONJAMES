...
async function getTotalXP() {
    try {
        ...
        // Fallback to direct API call if xpService not initialized
        // Try to fetch from backend
        const response = await fetch('https://financial-backend1.onrender.com/backend/dummy_xp.php', {
            method: 'GET',
            credentials: 'include',
            headers: {
        ...
        if (!response.ok) {
            console.warn(`[Debug] HTTP error! status: ${response.status}`);
            return 0;
        }
...
async function addXP(baseAmount, bonuses = {}) {
    try {
        ...
        // Use xpService if available
        if (window.xpService && window.xpService.initialized) {
            await window.xpService.addXP(earnedXP);
        } else {
...
(function setupXPSync() {
    ...
    async function syncXPWithServer() {
        ...
        // Use xpService if available
        if (window.xpService && window.xpService.initialized) {
            console.log('[Debug] Using xpService for XP sync');
            // XP sync is handled automatically by xpService
            return;
        }
        
        try {
            console.log('[Debug] Manual XP sync in progress...');
            
            // Get XP from backend
            const xpResponse = await getTotalXP();
