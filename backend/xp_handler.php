...
<body>
    ...
    <script>
        ...
        window.markLevelCompleted = function(currentLevel, passed) {
            ...
            if (!alreadyCompleted) {
                ...
                const isExam = topic && topic.subLevels.some(sl => 
                    parseFloat(sl.id) === subLevelId && sl.isExam === true
                );
                ...
                if (isExam) {
                    ...
                    if (!completedExams.includes(examKey)) {
                        ...
                        console.log(`[markLevelCompleted] Exam ${examKey} added to completedExams`);
                    }
                }
                ...
                if (window.ConnectionHelper && typeof window.ConnectionHelper.updateXP === 'function') {
                    ...
                    window.ConnectionHelper.getUserXP()
                        .then(result => {
                            ...
                            return window.ConnectionHelper.updateXP(currentXP, completedLevels, completedExams);
                        })
                        .then(result => {
                ...
                ConnectionHelper.getUserXP()
                    .then(data => {
                        ...
                        if (data && typeof data.xp !== 'undefined') {
                            ...
                            // IMPORTANT: Always sync completion data from server before rendering UI
                            // This ensures we have the most up-to-date data about completed exams
                        // This specific block might be redundant if syncCompletedDataFromServer already handles this robustly.
                        // However, keeping it ensures that even a direct call to loadUserXP attempts to reconcile.
                            if (data.completed_exams && Array.isArray(data.completed_exams)) {
                                const localExams = JSON.parse(localStorage.getItem('completedExams') || '[]');
                            // Using the server's version directly after sync is preferred, 
                            // but if loadUserXP is called independently, merging is safer.
                                const mergedExams = [...new Set([...localExams, ...data.completed_exams])];
                                localStorage.setItem('completedExams', JSON.stringify(mergedExams));
                            console.log('[DEBUG] (loadUserXP) Updated localStorage completedExams from server:', {
                                    fromServer: data.completed_exams,
                                    merged: mergedExams
                                });
                            }
                         if (data.completed_levels && Array.isArray(data.completed_levels)) {
                            ...
                            const serverLevels = data.completed_levels;
                            const mergedLevelsMap = new Map();

                            localLevels.forEach(level => mergedLevelsMap.set(`${level.topicId}-${level.subLevelId}`, level));
                            serverLevels.forEach(level => mergedLevelsMap.set(`${level.topicId}-${level.subLevelId}`, level));
                            
                            const mergedLevels = Array.from(mergedLevelsMap.values());
                            localStorage.setItem('completedLevels', JSON.stringify(mergedLevels));
                             console.log('[DEBUG] (loadUserXP) Updated localStorage completedLevels from server:', {
                                fromServer: serverLevels,
                                merged: mergedLevels
                            });
                        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // DEBUG LOG: Display stored completed exams on page load
            const initialStoredExams = JSON.parse(localStorage.getItem('completedExams') || '[]');
            console.log('[DEBUG] Initial completedExams in localStorage:', initialStoredExams);
            
            // First try to sync data from server
            syncCompletedDataFromServer().then(() => {
                // Then initialize the UI with potentially updated data
                initializeTopicsUI();
            }).catch(error => {
                console.error('Error during data sync:', error);
                // Initialize UI anyway with local data
                initializeTopicsUI();
            });
            ...
            // Add the database test button event listener
            const dbTestBtn = document.getElementById('dbTestBtn');
            if (dbTestBtn) {
                dbTestBtn.addEventListener('click', testDatabaseConnection);
            }
            ...
            if (syncBtn) {
                syncBtn.addEventListener('click', function() {
                    // Change button text to show sync in progress
                    ...
                    syncCompletedDataFromServer().then(() => {
                        // Re-render UI with the updated data
                        initializeTopicsUI();
                        
                        // Show success notification
                        showNotification('Progress data synced successfully ✓');
                        ...
                        showNotification('Failed to sync progress data ✗', false);
            ...
            function updateConnectionStatus() {
                ...
                statusIndicator.querySelector('i').className = isOnline ? 
                    'fas fa-wifi' : 
                    'fas fa-wifi-slash';
                    
                statusIndicator.querySelector('span').textContent = isOnline ? 
                    'Online' : 
                    'Offline';

                // If connection status changes to online, try to reload XP
                if (isOnline) {
                    loadUserXP();
                }
            }
