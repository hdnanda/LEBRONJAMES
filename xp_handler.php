...
$headers = getallheaders();
$username = isset($headers['X-Username']) ? $headers['X-Username'] : null;
...
if (!$username) {
    debug_log("Username not provided in header");
...
if ($row = $result->fetch_assoc()) {
    ...
    debug_log("Created new user", ['username' => $username, 'user_id' => $user_id]);
}

// Handle different request methods
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        debug_log("GET request - Fetching XP for user", ['username' => $username, 'user_id' => $user_id]);
    ...
    case 'POST':
        ...
        debug_log("POST request - Updating XP", ['raw_input' => $input, 'username' => $username]);
        ...
        $xp = (int)$data['xp'];
        debug_log("Processing XP update", ['username' => $username, 'new_xp' => $xp]);
        ...
        if ($check_result->num_rows === 0) {
            debug_log("Creating new user_progress record", ['username' => $username]);
            // Insert new record
            ...
            debug_log("Updating existing user_progress record", ['username' => $username]);
